                ...
                <div class="col-lg-12">
                    <div class="col-md-12">
                        <div class="overview-wrap">
                            <h2 class="title-1 fw-bold">Status Pembayaran Laundry</h2>
                        </div>
                            ...
                            <table class="table table-borderless table-data3 table-hover ">
                                ...
                                @foreach ($status_pembayaran as $status)
                                    <tbody>
                                        <tr>
                                            <td>{{ $status->id }}</td>
                                            <td>{{ $status->kode_status_pembayaran }}</td>
                                            <td>{{ $status->status_pembayaran }}</td>
                                            <td>
                                                <div class="table-data-feature">
                                                    <a class="item bg-warning" href="/statusPembayaran/{{ $status->id }}/edit">
                                                        <i class="zmdi zmdi-edit text-white"></i>
                                                    </a>
                                                    <a class="item bg-danger" href="/statusPembayaran/{{ $status->id }}/delete" onclick="return confirm('apakah yakin akan di hapus ?')">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <h2 class="title-1"><strong>Tambah</strong> Status Pembayaran</h2>
                </h5>
                <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="card-body card-block">
                    <form action="statusPembayaran/tambah" method="post" class="form-horizontal">
                        {{ csrf_field() }}
                        <div class="card-body card-block">
                            <div class="form-group">
                                <label for="company" class=" form-control-label">Kode Status</label>
                                <input name="kode_status_pembayaran" type="text" id="company" placeholder=""
                                    class="form-control" value="{{ $noUrutAkhir }}" readonly>
                            </div>
                            <div class="form-group">
                                <label for="company" class=" form-control-label">Status Pembayaran</label>
                                <input name="status_pembayaran" type="text" id="company" placeholder=""
                                    class="form-control">
                            </div>
                        </div>
