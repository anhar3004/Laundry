	...
	<body>
		...
		<div class="invoice-box">
					...
					<td colspan="2">
						<table>
							<tr>
								...
								<td>
									{{ $p->customers->nama }}.<br />
									{{ $p->customers->alamat }}<br />
									{{ $p->customers->no_hp }} <br>
                                    {{ $p->customers->email }}
								</td>
							</tr>
						</table>
            ...
            <table>
				...
				<tr class="details">
					<td>{{ $p->customers->nama }}</td>
					<td>{{ $p->pakets->paket }}</td>
                    <td>Rp. {{ number_format($p->pakets->tarif) }}</td>
                    <td>{{ $p->berat }}Kg</td>
                    <td>{{ $p->statuspesanans->status_pesanan }}</td>
                    <td>{{ $p->statusPembayarans->status_pembayaran }}</td>
				...
				<tr class="total">
					...
					<td><strong>Rp.{{ number_format($p->total) }}</strong> </td>
				</tr>
            </table>
		</div>
        @endforeach
